...
Binding authoring contract for every doc in `codex/design-guidelines/`. `validate.sh` enforces the mechanical subset (skeleton, tags, refs, length); everything else here is reviewed by humans. Read this whole file before writing or editing any guideline doc. This file obeys its own rules and passes `validate.sh`.
...
- Run `bash codex/design-guidelines/validate.sh <FILE>` while writing and the full `bash codex/design-guidelines/validate.sh` before done; exit 0 required. [E]
