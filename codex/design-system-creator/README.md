...
A pick-the-right-weight system for giving a repo its design **North Star** — one `design/` folder that tells every future session (UX, UI, content, dev) how this product looks, moves, and speaks. Same shape as `~/.codex/orchestrating/`: a router sizes the ask and runs the matching mode. Built on `~/.codex/design-guidelines/` (the generic corpus): the North Star is its **project-specific distillation** — decisions, not options. Once it exists, sessions load 2 small files instead of re-deriving taste from the 26-doc corpus every time.

## Start

Fresh Codex chat **in the target repo**:
`Read ~/.codex/design-system-creator/README.md and run it.`
...
note it once. Never compress request_user_input text — interview options carry consequences.
...
- ABSENT, repo has no UI code        → SCRATCH  — read ~/.codex/design-system-creator/modes.md §Scratch
- ABSENT, repo has UI code/screens   → EXTRACT  — read ~/.codex/design-system-creator/modes.md §Extract
- PRESENT, bounded ask (new component, token tweak, new platform) → UPDATE — read ~/.codex/design-system-creator/modes.md §Update
- PRESENT, direction change (rebrand, pivot, "feels wrong/stale") → REVAMP — read ~/.codex/design-system-creator/modes.md §Revamp
- PRESENT, no build ask / "challenge it"                          → CHALLENGE — read ~/.codex/design-system-creator/challenge.md
...
1. **Interview before invention.** User answers outrank model taste. Question bank + batching rules in `interview.md`; `request_user_input`, ≤4 questions per call, recommended default first. Never ask what the repo already answers.
2. **Corpus-grounded.** Every foundation choice loads its `~/.codex/design-guidelines/` doc first — JIT via that folder's `INDEX.md` task tables, never bulk-load. The North Star records the *decision*; the corpus keeps the *reasoning*.
...
- `ui-kit.md` — component inventory, spec format, states/a11y minimums, optional design workspace sync
...
- `~/.codex/orchestrating/` — UI-touching units load the target repo's `design/INDEX.md`; UI-heavy wishlist with no North Star → router suggests this system first.
- `~/.codex/cleaning/` — medium+ docs-refresh verifies `design/` claims against shipped UI and flags an overdue challenge cadence.
- `~/.codex/design-guidelines/` — the corpus underneath; load via its INDEX task tables only.
