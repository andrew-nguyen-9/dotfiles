...
## design workspace sync (optional — requires design workspace tooling in the session; absent → skip, note once)

With a design-tool login, the live kit can mirror into a design workspace project so the system is browsable outside the repo:
...
The repo stays the source of truth; the design workspace project is a viewport. No login / tool absent → skip, note it once, nothing else depends on it.
