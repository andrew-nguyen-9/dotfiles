...
- Preattentive features — hue, orientation, size, motion — are detected in <250 ms regardless of item count; anything else forces a serial scan at ~40–50 ms per item. One variable per preattentive channel; two channels conjoined (e.g., "red AND large") lose the pop-out and revert to serial search. [E]
