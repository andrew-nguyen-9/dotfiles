...
- **Stop-before-verdict quirk** (07-09→07-15, reproduced on haiku/sonnet/opus — dispatch-design, not model size): judges spend the whole tool budget investigating and stop before the scoring line. `SendMessage`-by-agentId "emit verdict only, no new tool calls" resumes from transcript at 0 extra tool-uses, every cycle. Fix at dispatch: a hard investigation cap + one reserved final-verdict turn.
- **Verify expensive acceptance statically** (07-15): a judge that launches a multi-minute job (full OCR, e2e) inside its turn burns the budget and returns NO verdict — resume can't help when the check *is* the time-sink. Tell judges to confirm costly criteria by "fix present in diff + self-check green", never by running the job.
- **A judge `unmet` on a *presence* claim is a checkable fact, not a judgment** (07-09, 07-15): a green deterministic selftest overrides a judge false-negative; a judge reading only a summary file mistakes "not surfaced here" for "not checked". D grep-verifies a presence `unmet` before spending a panel.
- **Panels earn their cost on load-bearing units** (07-05): heterogeneous lenses (shell/adversarial) on the foundation unit surfaced 3 validator edge cases no single judge caught. Judges are stateless one-shots — relaunching a fan-out after the 5h cap costs nothing (same prompts, no lost state).
- **Judges don't run `build`/`next` in a shared worktree** (07-10): parallel builds race on one `.next` → phantom failures; lean on the D-run gate.
