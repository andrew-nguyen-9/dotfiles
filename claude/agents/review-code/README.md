...
- 2026-07-09 blind-judge@D (17 dispatches, 14 units + 3 panels): most haiku-lens + some sonnet judges spent all ~15 tool-uses investigating and stopped BEFORE emitting the verdict text — the scoring line was lost. `SendMessage`-by-agentId "reply with only SCORES/VERDICT" resumed from transcript instantly (0 extra tool-uses) — cheap recovery, but budget the verdict turn or cap the investigation. Also: a haiku judge that couldn't grep a symbol returned `unmet` (false negative) — the deterministic selftest that checks the same invariant was green and correctly overrode it, exactly as "green test beats unverified judge-unmet" prescribes.
- 2026-07-10 blind-judge@D (11 units, no panels): the truncate-before-verdict quirk reproduced on the **opus** lens too — 6/11 stopped at ~16 tool-uses mid-investigation, no scoring line. Same `SendMessage` "emit verdict, no new tool calls" recovered all 6 at 0 extra tool-uses. So it's dispatch-design, not model-specific (now seen haiku/sonnet/opus) — the fix is a hard investigation cap + a reserved final verdict turn, not a bigger model. All 11 verdicts MET, zero disagreement with the deterministic gate; a parallel `next build` race across judges sharing one worktree's `.next` produced phantom build failures — judges should not run `build` in the shared tree, lean on the D-run gate.
- 2026-07-15 blind-judge@D (9 units, no panels): 8/9 returned clean first try; the stop-before-verdict quirk hit 1 (E4) — `SendMessage` "emit verdict only" recovered at 0 extra tool-uses (consistent with prior cycles). NEW failure mode: the E4 judge scored `unmet` on a spec check (2.3 misidentification "not in the summary") that D disproved in 2 greps — the check *ran* (`run_qa`→`build_coverage_report`), it just wasn't consolidated into the one summary CSV; a judge reading only the summary file mistakes "not surfaced here" for "not checked". Lesson: a judge `unmet` on a *presence* claim is a checkable fact, not a judgment call — D should grep-verify before escalating a panel (I did; no panel spent). Also: one judge (E1) burned its whole budget (261s, 25 tool-uses) attempting a ~5min full-OCR acceptance run and produced NO verdict — resume can't help when the *check itself* is the time-sink. Dispatch must tell judges to verify expensive acceptance criteria statically (fix present in diff + self-check green) and never launch a multi-minute job inside the judge turn.
