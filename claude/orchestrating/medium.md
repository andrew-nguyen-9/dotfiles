...
**Routed here from `wishlist.md`.** Arriving from a lite escalation: skip step 1 for the originally-traced task only (newly-emerged units still get traced); lite's WIP branch name = the feature branch. Turns out trivial / single unit → drop to `lite.md`. Turns out many independent units, multi-day, or context won't fit one window → escalate to `orchestrator.md` Session A — **escalation ritual:** await in-flight agent returns (dispatch nothing new), merge landed unit branches into the feature branch, write plan + unit statuses to `plan.md` (schema: `units:` done/in-flight/todo, each 1 line · `deps:` unit→unit · `feature-branch:` <name>), then merge the feature branch → main first (preserves A's integration-from-main model), and print an A kickoff naming `plan.md` as wishlist input + main as A's brownfield start. **Stale `.orchestrator/` in the repo** → it silently activates the orch enforcement hooks on this session; print the cleaning kickoff and resolve first.
...
4. **Integrate.** One feature branch; bring the unit work together, resolve conflicts, run the **full build + test + lint on the combined result** (not just per-unit). `think hard` only if a cross-unit break appears. Overlap surfaced mid-build (two units editing one file — step-3 prevention failed) → name one unit the file's owner; the other's overlapping hunks re-do on top after the owner merges.
5. **Verify + land.** DoD green (`verification-before-completion`: evidence, not assertion). Research/docs-only units: gate = the smallest runnable check that fails if the work is wrong — never a fake build-green. One unit red after integration → revert that unit's merge, land the green subset, log the dropped unit (mirrors orchestrator D ship-without). Trust-boundary diff (auth/input/secrets) → dispatch the matching security reviewer (`agents-docs/security/`) on it before land. `commit -q`, no AI attribution; PR only if asked. Then print the cleaning kickoff for a fresh chat: `Read ~/.claude/cleaning/README.md and run it.`
