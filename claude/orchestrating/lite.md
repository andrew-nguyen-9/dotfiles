...
**Routed here from `wishlist.md`.** If mid-task it grows past one sequential pass, escalate: a handful of units needing a plan + parallel work → `medium.md` (commit WIP to a branch, continue in the SAME chat reading medium.md); many independent units / multi-day / context won't fit → `orchestrator.md` Session A (your findings become its wishlist input). Don't grind a bigger job through this path.
