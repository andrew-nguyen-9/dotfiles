...
**Session read map (each session loads only its own file + shared spine sections):** A = this whole spine (it fills the blanks). B = `~/.claude/orchestrating/session-b.md` + spine §Tiers + §Invariants + §Thinking budget + §Swap. C = `~/.claude/orchestrating/session-c.md` + `~/.claude/orchestrating/efficiency.md` (skeleton) + the project copy's §Swap. D = `~/.claude/orchestrating/session-d.md` + `~/.claude/orchestrating/efficiency.md` §Safeguards (+ artifacts `depmap.md` + `progress.md` — fix-unit scoping + C's substitution/gap notes). Shared spine sections live in `.orchestrator/orchestrator.md` (the project copy, blanks filled).
...
- **Efficiency layer (§1–5, dispatch skeleton) + Safeguards** → `~/.claude/orchestrating/efficiency.md`
