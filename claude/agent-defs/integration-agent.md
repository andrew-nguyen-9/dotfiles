...
3. Full regression + integration/e2e + lint ON `integration` (not per-unit). Green-alone-but-red-together = cross-unit break: diagnose, fix if mechanical, else `git reset --hard <pre-wave SHA>` (never leave integration mutated+red) and fail with the SHA + suspected breaking pair. C must NOT branch the next wave until integration is green.
