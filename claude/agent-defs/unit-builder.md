...
- Fix mode: dispatch asks to merge into, fix, or amend an EXISTING `<prefix>/<unit>` (e.g. "merge integration into <prefix>/<unit>, resolve conflicts", "fix <failing test> on <prefix>/<unit>") → `git checkout <prefix>/<unit>` (no `-b`), do the merge/fix/resolution; the base-branch rules above — INCLUDING the no-base `status:"fail"` — do NOT apply (the branch already exists). The upstream GATE still applies.
