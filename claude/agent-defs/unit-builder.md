...
- TDD for non-trivial logic. DoD gate: build + test + lint green before done — evidence, not assertion. A dispatch `dod:` line overrides brief/CLAUDE.md commands (medium research/docs-only units carry it; running the repo build/test there is a fake green).
- Base branch = the dispatch's `base: <branch>` line; absent → `integration`. If neither `base:` given nor an `integration` branch exists → return `status:"fail"`, note asking for the base branch (mirror the upstream GATE — never guess a base). A `base:` given but unresolvable (branch deleted/moved) → return `status:"fail"`, note it; never substitute a guess. Branch name = the dispatch's `branch: <name>` line (medium tier sends it); absent → `<prefix>/<unit>` with prefix from brief/CLAUDE.md. One branch off that base. Your worktree's HEAD may NOT be the base — branch explicitly: `git checkout -b <prefix>/<unit> <base>` (diff against `<base>`, never bare HEAD). `commit -q`. No AI attribution. Secrets from the location named in brief/CLAUDE.md — never committed, never echoed.
