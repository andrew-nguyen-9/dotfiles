...
3. **Corpus load (JIT).** Open `~/.claude/design-guidelines/INDEX.md`, take the matching task rows, load ONLY those docs. Baseline for any new system: the "new brand / visual identity" row (PHILOSOPHY, COLOR_SYSTEMS, TYPOGRAPHY_SYSTEMS, ANTI_HOMOGENIZATION); add rows the Brief triggers (data-viz needed → DATA_VISUALIZATION; native apps → MULTI_DEVICE_ECOSYSTEMS; motion signature → MOTION_AND_SPATIAL_BEHAVIOR).
4. **Directions.** Draft 2–3 genuinely distinct design directions. Each = palette seed (5 hex) + type pairing + spacing base + one line tying it to the P1.2 anchor object. Present via `AskUserQuestion` with previews (one per direction); the pick becomes the system's spine. One direction must be the daring one — never three safe variants. Skip when P0.4 = full brand book (the book is the direction).
...
9. **Handoff.** Print the one paragraph future sessions need: where the North Star lives, the agent load map line, the challenge cadence. Then print the cleaning kickoff if this ran inside an orchestrating cycle.
...
- Mine first, ask second: census existing UI — CSS custom properties, tailwind/theme config, component directory listing, the 5 most-rendered screens (browser tools if reachable, else source). Build a `found | value | confidence` table.
- Interview per matrix: P1 + P4 full; P2–P3 presented as confirm-rounds seeded with mined values.
- **Code vs intent conflicts** (mined button uses 6 grays, user says "calm and minimal") are findings, not silent fixes: list them in the manifest as `codify-as-is` vs `correct-in-system` per row — the user picks. The North Star records intent; a `## Drift` list in DECISIONS names where shipped code disagrees, as future cleanup units.
- Components: inventory what exists (status `live`), spec the P3.1 gaps as `planned`. Never spec-from-imagination what code already answers.
...
Update discovering it needs new foundations → Revamp (say so, re-route mid-run). Extract finding no coherent existing UI (three unrelated ad-hoc styles) → Scratch with the census as P0 evidence. Any mode finding `design/` present but failing `validate.sh` → fix structure first, then run the routed mode.
