...
4. **Execute — bounded fan-out.** Partition the approved manifest into **disjoint path sets** → 2–4 fresh agents in parallel (e.g. docs-migration / junk+renames — NEVER docs-refresh here: it runs sequentially in step 5, after moves land, or it patches paths mid-migration), each returns a **≤2-line note**, never a dump. Overlapping paths → do those sequentially yourself. Disjoint partition = no worktree machinery needed.
