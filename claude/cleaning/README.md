...
3. **Ref-check before delete/move.** Code: serena `find_referencing_symbols` / grep imports — zero refs required — **plus grep the bare filename repo-wide** (catches shell `source`/`.` lines, Makefile/CI/settings.json references that symbol tools can't see). Docs: grep for links to the old path.
...
7. **Docs refresh** runs whenever medium/deep runs: verify README/CLAUDE.md/core-doc claims against actual code, patch stale sections. Also the global **agents catalog** (`~/.claude/agents-docs/` — symlinked, so any repo's run may touch it): verify rows vs installed plugins, fold sprawled `## Lessons` one-liners into table rows, prune dead agents. Repo has a `design/` North Star (`design/INDEX.md`) → run `bash design/validate.sh`, spot-check FOUNDATIONS/UI-KIT claims against shipped UI (drift → flag, don't rewrite), and flag an overdue `last-challenged:` stamp vs its cadence (fix = a `challenge` run via `~/.claude/design-system-creator/`, not a cleaning op).
