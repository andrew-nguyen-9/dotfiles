...
4. **Docs refresh.** README + CLAUDE.md/AGENTS.md + core docs: verify claims against code; harvested gotchas/decisions (lite step 3) fold into this pass — dedupe against what the docs already say (serena `get_symbols_overview`, package manifests, build/scripts). **Enforce the CLAUDE.md ≤20-line cap** (nobody else does): over → compress, overflow detail moves to docs/ with a pointer line. Prune `02 §Gotchas` lines whose path/symbol no longer exists (dead-ref = fixed or stale). Patch stale sections; big drift → flag in summary, don't rewrite the voice. Merged docs get one coherence pass. Agents catalog (`~/.claude/agents-docs/`): verify + consolidate per `README.md` §Invariants 7 — catalog edits dirty the dotfiles repo; commit them there (`chore:`).
